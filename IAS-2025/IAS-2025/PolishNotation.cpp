	...
	bool polishNotation(int lextable_pos, LT::LexTable& lextable, IT::IdTable& idtable)
	{
		...
		for (; lextable.table[i]->lexema != LEX_SEMICOLON && (lextable.table[i]->lexema != LEX_RIGHTHESIS || !stk.empty()); i++)
		{
			...
			case 'n': // >>
				...
				{
					// Приоритет сдвигов обычно ниже, чем у умножения/деления
					// но выше, чем у сложения/вычитания
					int prioritet = (lextable.table[i]->lexema == 'b' || lextable.table[i]->lexema == 'n') ? 5 : priority(lextable.table[i]->sign);
